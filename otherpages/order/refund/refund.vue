			...
			<scroll-view scroll-y="true" class="refund-container">
				<view class="goods-wrap">
					<view class="goods-img">
						<image :src="$util.img(refund_data.order_goods_info.sku_image, { size: 'mid' })" @error="refund_data.order_goods_info.sku_image = $util.getDefaultImage().default_goods_img"
				...
				<view class="refund-option" v-show="!refund_type">
					<view class="option-item" @click="selectRefundType(1)">
						<view>
							<text>退款无需退货</text>
							<text class="ns-font-size-sm ns-text-color-gray">没收到货，或与卖家协商同意无需退货只退款</text>
						</view>
					...
					<view class="option-item" @click="selectRefundType(2)" v-if="refund_data.refund_type.length == 2">
						<view>
							<text>退货退款</text>
							<text class="ns-font-size-sm ns-text-color-gray">已收到货，需退还收到的货物</text>
						</view>
						<text class="iconfont iconright"></text>
					</view>
				</view>
				...
				<view v-show="refund_type">
					<view class="refund-form">
						<view class="item-wrap" @click="openPopup('refundReasonPopup')">
							...
							<view class="cont">
								<text class="ns-text-color-gray" v-if="!refund_reason.length">请选择</text>
								<text class="ns-text-color-gray" v-else>{{ refund_reason }}</text>
							</view>
							<text class="iconfont iconright"></text>
						</view>
						<view class="item-wrap">
							<view class="label">退款金额：</view>
							<view class="cont ns-text-color">{{ $lang('common.currencySymbol') }}{{ refund_data.refund_money }}</view>
						</view>
						<view class="item-wrap">
						...
						<!-- #ifdef MP-WEIXIN -->
						<textarea v-if="!showText" class="newText" value="" placeholder="请输入退款说明(选填)" placeholder-class="ns-text-color-gray ns-font-size-base"
						 :auto-height="true" v-model="refund_remark" />
						<!-- #endif -->
						<!-- #ifdef H5 || APP-PLUS -->
							<textarea class="newText" value="" placeholder="请输入退款说明(选填)" @blur="textBlur()" placeholder-class="ns-text-color-gray ns-font-size-base" :auto-height="true"
								v-model="refund_remark" />
						<!-- #endif -->
					
					</view>
		
					<!-- 	<view class="sub-btn ns-bg-color" :class="{ 'safe-area': isIphoneX }" @click="submit">{{ $lang('common.submit') }}</view> -->
				...
				<uni-popup ref="refundReasonPopup" type="bottom" @change="change()">
					<view class="refund-reason-popup popup">
						<view class="popup-header">
							...
							<view class="align-right" @click="closePopup('refundReasonPopup')"><text class="iconfont iconguanbi ns-text-color-gray"></text></view>
						</view>
						<scroll-view scroll-y="true" class="popup-body" :class="{ 'safe-area': isIphoneX }">
							<view class="reason-list">
								<view class="item" v-for="(item, index) in refund_data.refund_reason_type" :key="index" @click="changeReason(item)">
									...
									<view class="iconfont" :class="refund_reason == item ? 'iconyuan_checked ns-text-color' : 'iconyuan_checkbox'"></view>
								</view>
							</view>
						</scroll-view>
						<view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }"><view class="confirm-btn ns-bg-color" @click="closePopup('refundReasonPopup')">确定</view></view>
					</view>
				</uni-popup>
			</scroll-view>
