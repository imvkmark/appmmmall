<template>
	<view class="live-content" :class="themeStyle">
		<!-- #ifdef MP-WEIXIN -->
				...
				<block v-if="liveList.length">
					<view class="live-wrap" v-for="(item, index) in liveList" :key="index" @click="entryRoom(item.roomid)">
						<view class="banner-wrap">
							<image :src="item.banner != '' ? $util.img(item.banner) : $util.img('upload/uniapp/live_default_banner.png')" mode="widthFix"></image>
							<view class="shade"></view>
							<view class="wrap">
								<view class="room-name">
									<text class="status-name" :class="{'ns-bg-color' : item.live_status == '101' }"><text class="iconfont iconzhibozhong" v-if="item.live_status == '101'"></text>{{ item.status_name }}</text>
									{{ item.name }}
								</view>
							</view>
						</view>
						<view class="room-info">
							<image :src="item.anchor_img != '' ? $util.img(item.anchor_img) : $util.getDefaultImage().default_headimg" @error="imageError(index)" mode="" class="anchor-img"></image>
							<text class="anchor-name">主播：{{item.anchor_name}}</text>
							<text class="separate">|</text>
							<text class="goods-text">直播商品：{{item.goods.length}}</text>
						</view>
						<view class="time">
							<text>{{ $util.timeStampTurnTime(item.start_time)}} <text class="separate">-</text> {{$util.timeStampTurnTime(item.end_time)}}</text>
						</view>
					</view>
				</block>
				<block v-else>
