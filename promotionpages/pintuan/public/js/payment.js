export default {
	...
	methods: {
		 ...
		 */
		selectAddress(){
			this.$util.redirectTo('/otherpages/member/address/address', {'back': '/promotionpages/pintuan/payment/payment'});
		},
		/**
		 * 获取订单初始化数据
		 */
							...
							uni.removeStorage({
							    ...
							    success: () =>  {
									if (this.orderPaymentData.pay_money == 0) {
										this.$util.redirectTo('/pages/pay/result/result', {code: res.data}, 'redirectTo');
									} else {
										this.$util.redirectTo('/pages/pay/index/index', {code: res.data}, 'redirectTo');
									}
							    }
							});
						} else {
		 ...
		 */
		setPayPassword(){
			this.$util.redirectTo('/otherpages/member/pay_password/pay_password', {back : '/promotionpages/pintuan/payment/payment'});
		},
		/**
		 * 暂不设置支付密码
		 */
