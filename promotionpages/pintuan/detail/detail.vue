				...
				<view class="goods-pintuan" v-if="goodsSkuDetail.timeMachine">
					<view class="price-info">
						<view class="discount-price">
							...
							<text class="pintuan-save-price">拼团省{{ $lang('common.currencySymbol') }} {{ goodsSkuDetail.save_price }}</text>
						</view>
						<view class="original-price">
							<text class="price">{{ $lang('common.currencySymbol') }} {{ goodsSkuDetail.price }}</text>
							<view class="pintuan-flag">
								<text class="iconfont iconpintuan"></text>
								<text>{{ goodsSkuDetail.pintuan_num }}人拼团</text>
							</view>
						</view>
					</view>
							...
							<view class="item">
								...
								<view class="info">
									<template v-if="item.timeMachine">
										<view class="tip">
											还差
											<text class="ns-text-color">{{ item.pintuan_num - item.pintuan_count }}人</text>
											拼成
										</view>
										<text>剩余</text>
										<uni-count-down
								...
								<template v-else>
									<button class="btn-disabled" type="default" size="mini">已结束</button>
								</template>
							</view>
						</swiper-item>
					</swiper>
						...
						<uni-popup ref="pintuanPopup" type="center" class="pintuan-popup-layer">
							<view class="layer">
								...
								<view class="info" v-if="currentPintuan">
									仅剩
									<text class="ns-text-color">{{ currentPintuan.pintuan_num }}</text>
									个名额，
									<uni-count-down
										v-if="openPopup"
								...
								<view class="user-list">
									<view class="item">
										<text class="boss ns-bg-color">拼主</text>
										<image
								...
								<button type="primary" @click="joinPintuan()" class="mini-btn" size="mini">参与拼单</button>
							</view>
						</uni-popup>
					</view>
				</view>
				...
				<view class="group-wrap">
					<view class="goods-evaluate">
						<view class="tit">
							...
							<navigator hover-class="none" class="ns-text-color" :url="'/otherpages/goods/evaluate/evaluate?goods_id=' + goodsSkuDetail.goods_id">
								<text>查看更多</text>
								<text class="iconfont iconright"></text>
							</navigator>
						</view>
						<view class="evaluate-item" v-if="goodsEvaluate.content">
							<view class="evaluator">
								<view class="evaluator-face">
									...
									<image
										v-else
										:src="$util.img($util.getDefaultImage().default_headimg)"
										mode="aspectFill"
									/>
								</view>
