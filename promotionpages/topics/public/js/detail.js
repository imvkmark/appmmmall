	...
	methods: {
		...
		goConnect(){
			...
			if (uni.getStorageSync('token')) {
				...
				that.$util.redirectTo('/pages/login/login/login', {
					back: '/promotionpages/topics/goods_detail/goods_detail?id=' + that.id
				});
			}
		},
		// 获取专题活动商品详情
		async getGoodsSkuDetail() {
			...
			if (data.goods_sku_detail != null) {
				...
				this.contactData = {
					title: this.goodsSkuDetail.sku_name,
					path: '/promotionpages/topic/detail/detail?id=' + this.id,
					...
					success: res => {
						if (res.code == 0) {
							...
							let shareConfig = res.data.share_config,
								url = this.$config.h5Domain + '/promotionpages/topic/detail/detail?id=' + this.id;
