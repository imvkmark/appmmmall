<template>
	<view class="order-container" :class="themeStyle">
		...
		<view class="address-wrap" v-if="orderPaymentData.is_virtual == 0">
			<view class="icon">
				<view class="iconfont icondizhi ns-text-color"></view>
			</view>
		...
		<view class="mobile-wrap" v-if="orderPaymentData.is_virtual == 1">
			...
			<view class="form-group">
				<view class="form-item">
					...
					<input type="number" value="" maxlength="11" placeholder="请输入您的手机号码" placeholder-class="ns-font-size-base ns-text-color-gray placeholder"
					 class="ns-font-size-base input" v-model="orderCreateData.member_address.mobile" />
				</view>
			</view>
		</view>
		...
		<view class="site-wrap">
			...
			<view class="site-footer">
				...
				<view class="order-cell">
					...
					<view class="box">
						<input type="text" value="" placeholder="留言前建议先与商家协调一致" class="ns-text-color-gray ns-font-size-base"
						 placeholder-class="ns-text-color-gray ns-font-size-base" v-model="orderCreateData.buyer_message" />
					</view>
				</view>
			</view>
		</view>
		...
		<!-- 付款台 -->
		<view class="order-checkout" v-if="orderPaymentData.member_account&&orderPaymentData.member_account.balance_total > 0">
			<view class="order-cell">
		...
		<uni-popup ref="invoicePopup" type="bottom">
			<view class="invoice-popup popup">
					...
					<view>
						<view class="invoice-cell">
							...
							<view class="option-grpup">
								<view class="option-item active ns-text-color ns-border-color">普通发票</view>
						...
						<view class="invoice-cell">
							...
							<view class="option-grpup">
								<view class="option-item active ns-text-color ns-border-color">个人</view>
								<view class="option-item">企业</view>
							</view>
							<view class="form-group">
								<view class="form-item">
									<text class="text">单位名称</text>
									<input type="text" value="" placeholder="请填写单位名称" placeholder-class="ns-font-size-base ns-text-color-gray placeholder"
									 class="ns-font-size-base input" />
								</view>
								<view class="form-item">
									<text class="text">纳税人识别号</text>
									<input type="text" value="" placeholder="请填写纳税人识别号" placeholder-class="ns-font-size-base ns-text-color placeholder"
									 class="ns-font-size-base input" />
								</view>
							</view>
						</view>
						<view class="invoice-cell">
							...
							<view class="option-grpup">
								<view class="option-item active ns-text-color ns-border-color">商品明细</view>
				...
				<view class="popup-footer">
					<view class="confirm-btn ns-bg-color">确定</view>
				</view>
			</view>
		</uni-popup>
		...
		<uni-popup ref="couponPopup" type="bottom">
			<view class="coupon-popup popup">
				...
				<scroll-view scroll-y="true" class="popup-body">
					<view class="coupon-item" v-for="(couponItem, couponIndex) in orderPaymentData.shop_goods_list.coupon_list" :key="couponIndex"
				...
				<view class="popup-footer">
					<view class="confirm-btn ns-bg-color" @click="popupConfirm('couponPopup')">确定</view>
				</view>
			</view>
		</uni-popup>
		<!-- 活动优惠弹窗 -->
		<uni-popup ref="sitePromotionPopup" type="bottom">
			<view class="promotion-popup popup">
				...
				<scroll-view scroll-y="true" class="popup-body">
					<view>
						<view class="order-cell">
							<text class="tit"><text class="promotion-mark ns-gradient-promotionpages-topics-payment" :class="themeStyle">专题活动</text>{{ orderPaymentData.topic_info.topic_name }}</text>
							<view class="box align-right">
							</view>
						</view>
					</view>
			...
			<view class="delivery-popup popup">
				...
				<view class="popup-body">
					<view class="delivery-cell">
						<view class="option-grpup">
							<view class="option-item" :class="{ 'active ns-text-color ns-border-color': deliveryItem.name == orderPaymentData.delivery.delivery_type }"
							 v-for="(deliveryItem, deliveryIndex) in orderPaymentData.shop_goods_list.express_type" :key="deliveryIndex"
						...
						<block v-if="deliveryItem.name == 'store'">
							<view class="delivery-cell delivery-cont" v-show="orderPaymentData.delivery.delivery_type == 'store'">
								<block v-if="deliveryItem.store_list.length">
									<view class="pickup-point" v-for="(storeItem, storeIndex) in deliveryItem.store_list" :key="storeIndex" @click="selectPickupPoint(storeItem.store_id)">
										<view class="name">
		...
		<uni-popup ref="payPassword" :custom="true">
			<view class="pay-password">
				<block v-if="orderPaymentData.member_account&&orderPaymentData.member_account.is_pay_password == 0">
