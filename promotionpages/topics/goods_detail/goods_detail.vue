<template>
	<view :class="themeStyle">
		<!-- #ifdef APP-PLUS -->
		<view @click="$util.goBack()" class="iconfont iconback_light diy-back-btn"></view>
		<!-- #endif -->
...
<!-- 播放视频 -->
				...
				<view class="topic-wrap" v-if="goodsSkuDetail.discountTimeMachine">
					<view class="promotion-wrap">
						<text class="iconfont iconZ-daojishi"></text>
						<text class="txt">{{ goodsSkuDetail.topic_name }}</text>
					</view>
					<view class="countdown">
						<view class="txt ns-text-color">距结束仅剩</view>
						<view class="clockrun">
							<uni-count-down
				...
				<view class="group-wrap">
					...
					<view class="goods-module-wrap">
						<view class="price-wrap">
							<view class="topic-price ns-text-color">
								<text class="symbol">{{ $lang('common.currencySymbol') }}</text>
								<text class="price">{{ goodsSkuDetail.topic_price }}</text>
							</view>
							<view class="original-price">
								<text class="price">{{ $lang('common.currencySymbol') }} {{ goodsSkuDetail.price }}</text>
								<text class="topic-save-price">立省{{ $lang('common.currencySymbol') }}{{ goodsSkuDetail.save_price }}</text>
							</view>
							<view class="sale-num ns-text-color">销量：{{ goodsSkuDetail.sale_num }}{{ goodsSkuDetail.unit }}</view>
						</view>
					</view>
				</view>
						...
						<view class="shop-wrap">
							...
							<view class="box">
								<view class="goods-action">
									<navigator hover-class="none" class="ns-border-color ns-text-color" :url="'/otherpages/shop/list/list?site_id=' + shopInfo.site_id">全部商品</navigator>
									<navigator hover-class="none" class="ns-border-color ns-text-color" :url="'/otherpages/shop/index/index?site_id=' + shopInfo.site_id">查看店铺</navigator>
								</view>
							</view>
						</view>
					</view>
				...
				<!-- SKU选择 -->
				<ns-goods-sku ref="goodsSku" :disabled="true" :goods-detail="goodsSkuDetail"></ns-goods-sku>
		...
		<ns-goods-action>
			<template v-if="goodsSkuDetail.goods_state == 1 && goodsSkuDetail.verify_state == 1">
				...
				<ns-goods-action-button class="goods-action-button active1" text="立即抢购" @click="topic" />
			</template>
			<template v-else>
...
export default {
	...
	onShareAppMessage(res) {
		var path = '/promotionpages/topic/detail/detail?id=' + this.id;
...
.topic-wrap .countdown .clockrun >>> .uni-countdown__number {
	min-width: 32rpx;
	height: 32rpx;
	text-align: center;
	line-height: 32rpx;
...
.topic-wrap .countdown .clockrun >>> .uni-countdown__splitor {
	height: 32rpx;
	line-height: 36rpx;
	text-align: center;
	display: inline-block;
...
.topic-wrap .countdown .clockrun >>> .uni-countdown__splitor.day {
	width: initial;
}
