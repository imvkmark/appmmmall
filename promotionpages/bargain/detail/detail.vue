	...
	<view :data-theme="themeStyle">
		<view class="goods-detail" :class="isIphoneX ? 'active' : ''">
			...
			<view class="goods-container">
				...
				<view class="goods-pintuan" v-if="goodsSkuDetail.timeMachine">
					<view class="price-info">
						<view class="price-info-box">
							<view class="groupbuy-price">
								<text class="symbol">{{ $lang('common.currencySymbol') }}</text>
								<text class="price-big">{{IntPrice}}</text>
								<text class="price-min">{{floatPrice}}</text>
							</view>
							<view class="line-price-box">
								<view class="line-price-detail">
									<text class="symbol ns-font-size-lg">{{ $lang('common.currencySymbol') }}</text>
									<text class="line-price">{{ goodsSkuDetail.price }}</text>
								</view>
								<view class="line-price-bottom">
									<text class="group-price-name ns-text-color">砍价享低价</text>
								</view>
							</view>
						</view>
					</view>
				...
				<view class="newdetail ns-margin-bottom">
					<!-- 服务 -->
					<!-- <view class="newdetail-item service" @click="openMerchantsServicePopup()" v-if="goodsSkuDetail.goods_service&&goodsSkuDetail.goods_service.length">
				...
				<!-- 店铺信息 -->
				<block v-if="Development">
					<view class="shop-wrap" v-if="preview == 0">
						...
						<view class="box">
							<view class="goods-action">
								<navigator hover-class="none" class="ns-text-color ns-border-color" :url="'/otherpages/shop/list/list?site_id=' + shopInfo.site_id">全部商品</navigator>
								<navigator hover-class="none" class="ns-text-color ns-border-color" :url="'/otherpages/shop/index/index?site_id=' + shopInfo.site_id">查看店铺</navigator>
							</view>
						</view>
					</view>
				</block>
					...
					<template v-if="goodsSkuDetail.goods_state == 1">
						...
						<block v-else>
							<ns-goods-action-button class="goods-action-button active4"
							 :text-price="'最低¥ ' + goodsSkuDetail.show_price" text="去砍价" @click="bargain" v-if="goodsSkuDetail.bargain_stock > 0"/>
							<ns-goods-action-button class="goods-action-button active3" disabled-text="库存不足" :disabled="true" v-else/>
						</block>
					</template>
					<template v-else>
...
<script>
	...
	export default {
		...
		computed: {
			IntPrice() {
				if (this.goodsSkuDetail && this.goodsSkuDetail.show_price) {
					return this.goodsSkuDetail.show_price.toString().slice(0, -3)
				} else {
					return '0'
				}
			...
			floatPrice() {
				if (this.goodsSkuDetail && this.goodsSkuDetail.show_price) {
					return this.goodsSkuDetail.show_price.toString().slice(-3)
				} else {
					return '.00'
				}
			}
		...
		onShareAppMessage(res) {
			var path = '/promotionpages/pintuan/detail/detail?id=' + this.id + '&group_id=' + this.groupId;
...
<style scoped>
	...
	.goods-pintuan .countdown .clockrun>>>.uni-countdown__number {
		min-width: 32rpx;
		height: 32rpx;
		text-align: center;
		line-height: 32rpx;
	...
	.goods-pintuan .countdown .clockrun>>>.uni-countdown__splitor {
		width: 10rpx;
		height: 32rpx;
		line-height: 36rpx;
		text-align: center;
	...
	.goods-pintuan .countdown .clockrun>>>.uni-countdown__splitor.day {
		width: initial;
	}
