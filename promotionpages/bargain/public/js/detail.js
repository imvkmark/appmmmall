	...
	methods: {
		...
		goConnect(){
			...
			if (uni.getStorageSync('token')) {
				...
				that.$util.redirectTo('/pages/login/login/login', {
					back: '/promotionpages/bargain/detail/detail?id=' + that.id
				});
			}
		},
		// 获取砍价商品详情
		async getGoodsSkuDetail() {
			...
			if (data.goods_sku_detail != null) {
				...
				this.contactData = {
					title: this.goodsSkuDetail.sku_name,
					path: '/promotionpages/bargain/detail/detail?id=' + this.id,
		...
		bargain() {
			...
			this.$api.sendRequest({
				...
				success: res => {
					if (res.code == 0) {
						this.$util.redirectTo('/promotionpages/bargain/launch/launch', {id: res.data});
					} else {
						this.$util.showToast({
							title: res.message
						});
