				...
				<view class="goods-list" :class="listStyle">
					<view v-for="(item, index) in goodsList" :key="index" class="goods-item" @click="navToDetailPage(item)">
						...
						<view class="goods-info">
							...
							<view class="is_discount ns-border-color ns-text-color" v-if="item.promotion_type == 1 && addonIsExit.discount">
								限时折扣
							</view>
							...
							<view class="other-info">
								<text v-if="item.is_free_shipping == 1">免运费</text>
							</view>
				...
				<view>
					...
					<view v-for="(item, index) in categoryList" :key="index" class="list-wrap">
						...
						<view v-for="(second_item, second_index) in item.child_list" :key="second_index">
							<text class="second" @click="selectedCategory(second_item.category_id, second_item.level)" :class="{ 'ns-text-color': second_item.category_id == categoryId }">
		...
		<uni-drawer :visible="showScreen" mode="right" @close="showScreen = false" class="screen-wrap">
			...
			<scroll-view scroll-y>
				...
				<view class="item-wrap">
					...
					<view class="list">
						<uni-tag :inverted="true" text="包邮" :type="isFreeShipping>0 ? 'primary' : 'default'" @click="isFreeShipping = isFreeShipping===-1||isFreeShipping===0?1:0" />
					</view>
				</view>
