<template>
	<view class="detail-container" :class="{ themeStyle, 'safe-area': isIphoneX }">
		...
		<view class="address-wrap active">
			<view class="icon"><view class="iconfont icondizhi"></view></view>
			<view class="address-info">
				<view class="info">
					<text>收货人：{{ orderData.name }}</text>
					<text>{{ orderData.mobile }}</text>
				</view>
				<view class="detail">
					<text>收货地址：{{ orderData.full_address }} {{ orderData.address }}</text>
				</view>
			</view>
		</view>
						...
						<view class="goods-operation">
							<block v-if="orderData.is_enable_refund">
								<!-- <navigator  hover-class="none"
									:url="'/otherpages/order/refund/refund?order_goods_id=' + goodsItem.order_goods_id"
									v-if="goodsItem.refund_status == 0 || goodsItem.refund_status == -1"
								> -->
								<navigator
									hover-class="none"
									:url="'/otherpages/order/refund/refund?order_goods_id=' + goodsItem.order_goods_id"
									v-if="goodsItem.refund_status == 0"
								>
									<view class="order-box-btn">退款</view>
								</navigator>
								<navigator hover-class="none" :url="'/otherpages/order/refund_detail/refund_detail?order_goods_id=' + goodsItem.order_goods_id" v-else>
		...
		<view class="order-operation bottom-safe-area" v-if="orderData.action.length > 0">
			<view class="order-box-btn white" v-if="orderData.is_evaluate == 1" @click="operation('memberOrderEvaluation')">
				<text v-if="orderData.evaluate_status == 0">评价</text>
				<text v-else-if="orderData.evaluate_status == 1">追评</text>
			</view>
			<view class="order-box-btn white" v-for="(operationItem, operationIndex) in orderData.action" :key="operationIndex" @click="operation(operationItem.action)">
				{{ operationItem.title }}
			</view>
		</view>
