<template>  
    <view class="container">  
		<view class="pay-group" v-for="(item, i) in list" :key="i"><!-- :key="item.configPaymentInfo.code" -->
			<view class="header">
			...
			<view v-for="(info, j) in item.payInfos" :key="info.id" class="item" :class="'item-bg-' + info.type">
				...
				<view class="center"  @click="handleView(item, info.id)">
					...
					<view class="qrcode" v-if="item.configPaymentInfo.type != 0">
						<image src="../../static/otc/icon-qrcode.png"></image>
					</view>
				</view>
			</view>
    ...
    export default {
		...
		onNavigationBarButtonTap(e) {
			...
			uni.showActionSheet({
			    ...
			    success: function (res) {
			        let i = res.tapIndex
					uni.navigateTo({
						url: "/pages/otc/updatePayType?data=" + encodeURIComponent(JSON.stringify($this.list[i].configPaymentInfo))
					})
			    },
			    fail: function (res) {
			    }
        ...
        methods: {
			...
			handleView(item, id){
				...
				uni.navigateTo({
					url: "/pages/otc/updatePayType?data=" + encodeURIComponent(JSON.stringify(item.configPaymentInfo))
				})
			},
			handleActive(id){
...
<style lang='scss' scoped>
	...
	.pay-group{
		...
		.item-bg-Alipay{
			background: linear-gradient(to left, rgba(55, 131, 217,0.7), rgba(55, 131, 217,1));
		}
		.item-bg-UnionPay{
			background: linear-gradient(to left, rgba(255,0,0,0.7), rgba(255,0,0,1));
		}
		.item-bg-Wechat{
			background: linear-gradient(to left, rgba(42, 189, 119,0.7), rgba(42, 189, 119,1));
		}
		.item{
			margin: 20upx 0upx;
			padding: 30upx 30upx;
