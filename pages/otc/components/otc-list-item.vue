<template>
	<view class="order-item little-line">
		<view style="padding:20upx 30upx;box-sizing: border-box;border:1upx solid #EEEEEE;">
			...
			<view class="row opt">
				<view class="pay">
					<image src="/static/otc/pay-alipay.png"></image>
					<image src="/static/otc/pay-wechat.png"></image>
					<image src="/static/otc/pay-paypal.png"></image>
					<!-- <i v-for="(t, index) in JSON.parse(data.payment)" :key="index" :class="`fexfont icon-${t}`"></i> -->
				</view>
				<view>
					<button class="btn buy" v-if="data.side == 'BUY'" @click="buy" style="margin:0 !important;">购买</button>
					<button class="btn sell" v-if="data.side == 'SELL'" @click="sell" style="margin:0 !important;">出售</button>
				</view>
			</view>
		</view>
		<!-- <u-keyboard ref="uKeyboard" @confirm="priceCancel" @cancel="priceCancel" @backspace="backspace" @change="priceChange" mode="number" :mask="true" v-model="showKeyboard" v-if="is_show"> -->
